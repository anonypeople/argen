...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:redirect="http://xml.apache.org/xalan/redirect"
    ...
    <xsl:template match="testsuite" mode="project.details">
        ...
        <xsl:variable name="project.name">
            <xsl:if test="not($directory.name = '')"><xsl:value-of select="$directory.name"/>.
            </xsl:if>
            ...
            <body>
                <table width="100%">
                    <tr>
                        <td nowrap="nowrap">
                            <h2>
                                <a href="directory-summary.html" target="projectFrame">
                                    <xsl:value-of select="$name"/>
                ...
                <table width="100%">
                    <xsl:for-each select="/testsuites/testsuite[./@package = $name]">
                        ...
                        <tr>
                            <td nowrap="nowrap">
                                <a href="{@id}_{@name}.html" target="projectFrame">
                                    <xsl:value-of select="@name"/>
                                </a>
                            </td>
                        </tr>
    ...
    <xsl:template match="testsuites" mode="all.directories">
        <html>
            ...
            <body>
                <h2>
                    <a href="overview-summary.html" target="projectFrame">Home</a>
                </h2>
                <h2>Directories</h2>
                <table width="100%">
            ...
            <body>
                ...
                <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
                    ...
                    <xsl:for-each select="testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
                        ...
                        <xsl:variable name="insamedirectory"
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <a href="./{translate(@package,'.','/')}/directory-summary.html">
                                    <xsl:value-of select="@package"/>
                                    <xsl:if test="@package = ''">&lt;none&gt;</xsl:if>
                                </a>
                            </td>
                            <td>
                                <xsl:value-of select="sum($insamedirectory/tests/text())"/>
                            </td>
                            <td>
                                <xsl:value-of select="sum($insamedirectory/errors/text())"/>
                            </td>
                            <td>
                                <xsl:value-of select="sum($insamedirectory/failures/text())"/>
                            </td>
                            <td>
                                <xsl:call-template name="display-time">
                            ...
                            <td>
                                <xsl:value-of select="$insamedirectory/@timestamp"/>
                            </td>
                            <td>
                                <xsl:value-of select="$insamedirectory/@hostname"/>
                            </td>
                        </tr>
                    </xsl:for-each>
                </table>
    ...
    <xsl:template name="create.stylesheet.link">
        ...
        <link rel="stylesheet" type="text/css" title="Style">
            <xsl:attribute name="href">
                <xsl:if test="not($directory.name = 'unnamed directory')">
                    <xsl:call-template name="path">
    ...
    <xsl:template match="testsuite" mode="print.test">
        <tr valign="top">
            ...
            <td>
                <a title="Display all tests" href="{@id}_{@name}.html">
                    <xsl:apply-templates select="tests/text()"/>
                </a>
            </td>
            <td>
                <xsl:choose>
            ...
            <td>
                ...
                <xsl:choose>
                    <xsl:when test="boolean($show.project)">
                        <a href="{concat($project.href, '#', @name)}">
                            <xsl:value-of select="@name"/>
                        </a>
                    </xsl:when>
                    <xsl:otherwise>
