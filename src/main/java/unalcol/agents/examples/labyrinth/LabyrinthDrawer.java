    ...
    public void paint(Graphics g) {
        if (environment != null && ((Labyrinth) environment).structure != null) {
            ...
            for (int i = 0; i < this.environment.agentsNumber(); i++) {
                ...
                switch (direction) {
                    case 0:
                        g.drawOval(X + CELL_SIZE / 2 - EYE_SIZE / 2, Y, EYE_SIZE, EYE_SIZE);
                        break;
                    case 1:
                        g.drawOval(X + CELL_SIZE - EYE_SIZE, Y + CELL_SIZE / 2 - EYE_SIZE / 2, EYE_SIZE, EYE_SIZE);
                        break;
                    case 2:
                        g.drawOval(X + CELL_SIZE / 2 - EYE_SIZE / 2, Y + CELL_SIZE - EYE_SIZE, EYE_SIZE, EYE_SIZE);
                        break;
                    case 3:
                        g.drawOval(X, Y + CELL_SIZE / 2 - EYE_SIZE / 2, EYE_SIZE, EYE_SIZE);
                        break;
                }
            }
        }
