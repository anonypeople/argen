...
public class SudokuPercept extends Percept {
    ...
    public void draw(Graphics g, int x, int y, int CELL_SIZE, SimpleLanguage language) {
        if (((Boolean) getAttribute(language.getPercept(0))).booleanValue()) {
            g.drawLine(x, y, x + CELL_SIZE, y);
        }
        if (((Boolean) getAttribute(language.getPercept(1))).booleanValue()) {
            g.drawLine(x + CELL_SIZE, y, x + CELL_SIZE, y + CELL_SIZE);
        }
        if (((Boolean) getAttribute(language.getPercept(2))).booleanValue()) {
            g.drawLine(x, y + CELL_SIZE, x + CELL_SIZE, y + CELL_SIZE);
        }
        if (((Boolean) getAttribute(language.getPercept(3))).booleanValue()) {
